...
def print_md5_timestamp_(filename):
	try:
		md5_hash = hashlib.md5()
		# Read and update hash in chunks of 4K
		with open(os.path.join('../server/shared') + '/' + filename,"rb") as f:
			for byte_block in iter(lambda: f.read(4096),b""):
				md5_hash.update(byte_block)
			checksum = md5_hash.hexdigest()
		cmd = 'date -r ' + os.path.join('../server/shared') + '/' + filename	
		mod_date = str(subprocess.check_output(cmd, shell=True))
	except OSError:
		print("OSError")
	print("Hash: ", checksum)
	print("Lastmodified time: ",mod_date)
...
def IndexGetlonglist():
	...
	print(subprocess.check_output('find ./shared -type f -name "*.txt" | while read f;do if grep -qF "Programmer" "$f";then ls -l "$f";fi;done',shell=True,text=True))
...
def FileHash_verify(filename):
	try:
		...
		print("File is not present in client side")
	except TypeError:
		print("File is not present in client side")	
		...
		if cmd[:8] == 'IndexGet': 
			...
			if cmd.split(' ')[1] == 'Shortlist':
				...
				try:
					...
					print("Time not entered in proper format")
				except ValueError:
					print("Time not entered in proper format")
				except IndexError:
					print("Time not entered in proper format")
...
try:
	...
	print("Please correct Input format")
